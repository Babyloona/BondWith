...
mongoose.connect("mongodb://127.0.0.1:27017/BondWith")
...
const House = mongoose.model('event', homeSchema)//модель на основе схемы
...
app.post('/',(async(req, res) => {
  if(req.isAuthenticated()){
    ...
    let home = await House.find({city: req.body.city, limitPeople:req.body.limitPeople})
      res.render('index', {req: req,home:home,user:user})
  }else{
    console.log(req.body.time);
    let home = await House.find({city: req.body.city, limitPeople:req.body.limitPeople})
      res.render('index', {req: req,home:home})
     ...
     let user=req.params.username
      const d = new Date();
       const home = new House({
        ...
        publishtime: d.toLocaleString('en-US', {day:'numeric', month: 'numeric', year: 'numeric'}),
...
app.post("/read/:_id",(async(req,res)=>{
  if(req.isAuthenticated()){
    let user= req.user
    let id=req.params._id
    let home = await House.findOne({_id:id})
...
try {
  ...
  let homes= await House.findOne({_id:homeid})
      ...
      const save = new Save({
       ...
       limitPeople:homes.limitPeople,
       myFile: homes.myFile ,
       author:homes.author,
       me:user,
   location:homes.location,
  //  event date
  eventtime:homes.eventtime,
  publishtime:homes.publishtime,
     clock:homes.clock
   })
   save.save()
res.redirect("/");
...
app.listen(9000,()=>{
    console.log(`9000 gotov k robote.GoooodLuck`)
